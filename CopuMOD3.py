...
class MultivariateSimulator:
    ...
    def fit_gaussian_copula_multivariate(self):
        ...
        # Ajustar modelo de cópula Gaussiana
        ...
        if method == 'gaussian_copula':
            ...
            for i in range(n_realizations):
                ...
                if preserve_marginals:
                    ...
                    for j, col in enumerate(self.available_cols):
                        sim_data[:, j] = self._inverse_cdf_empirical(u_sim[:, j], self.data[col].values)
                else:
                    sim_data = u_sim
            ...
            for i in range(n_realizations):
                ...
                if preserve_marginals:
                    ...
                    for j, col in enumerate(self.available_cols):
                        sim_data[:, j] = self._inverse_cdf_empirical(
                            stats.rankdata(data_original_scale[:, j]) / (len(data_original_scale) + 1),
    ...
    def plot_realizations_multivariate(self, var1, var2, realizations, n_show=50):
        ...
        fig.add_trace(go.Scatter(
            ...
            mode='markers',
            marker=dict(size=8, color='blue', opacity=0.5, symbol='circle'),
            name='Datos Originales'
        ))
        
        # Mostrar realizaciones (submuestra)
        n_show = min(n_show, len(realizations))
        ...
        for i in range(n_show):
            fig.add_trace(go.Scatter(
                ...
                mode='markers',
                marker=dict(size=4, color=colors[i % len(colors)], opacity=0.3),
                name=f'Realización {i+1}',
                showlegend=False
            ))
        ...
        with tab3:
            ...
            with col3:
                simulation_method = st.selectbox(
                    ...
                    format_func=lambda x: {
                        'gaussian_copula': 'Cópula Gaussiana Multivariada',
